...
impl Forest {
    pub fn new(path: &str, xpub: ExtendedPubKey) -> Result<Self, Error> {
    ...
    pub fn insert_path(&self, script: &Script, path: &Path) -> Result<(), Error> {
        Ok(self
            .paths
            .insert(script.as_bytes(), path.as_ref())
            .map(|_| ())?)
    }

    pub fn insert_index(&self, int_or_ext: u32, value: u32) -> Result<(), Error> {
        Ok(self
            .singles
    ...
    pub fn _get_index(&self, int_or_ext: u32) -> Result<u32, Error> {
        let ivec = self
            .singles
            .get([int_or_ext as u8])?
            .ok_or_else(fn_err("no index"))?;
